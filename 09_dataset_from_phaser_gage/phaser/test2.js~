...
function create() {
    ...
    pause_label = game.add.text(w - 100, 20, 'Pausa', { font: '20px Arial', fill: '#fff' });
...
function train_nn(){
    ...
    nn_trainer.train(trainingData, {rate: 0.0003,iterations: 10000,shuffle: true});
...
function get_op_from_trainedData(input_param){
    ...
    console.log("Valores ","En el aire %: "+ on_air + " En el piso %: " + on_floor );
...
function update() {
    ...
    if( auto_mode==true && bullet.position.x>0       &&
        player.body.onFloor()     ){
    ...
    if( auto_mode==false      &&
        ...
        trainingData.push({
                'entrada' :  [bullet_displacement , bullet_speed],
                'salida':  [stay_on_air , stay_on_floor ] 
        });

        console.log("Desplazamiento Bala, Velocidad Bala, En el aire, En el suelo: ",
